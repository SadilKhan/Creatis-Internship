...
def train(args, train_transforms=False, load=False):
    global Path
    logs_dir = args.logs_dir / args.name
    logs_dir.mkdir(exist_ok=True, parents=True)
    if args.expr_type == "smote":
        train_loader = DataLoader(PointCloudDataset(
            args.csvPath, transforms=train_transforms, mode="train", expr_type="smote"), batch_size=args.batch_size)
    elif args.expr_type=="adasyn":
        train_loader = DataLoader(PointCloudDataset(
            args.csvPath, transforms=train_transforms, mode="train", expr_type="adasyn"), batch_size=args.batch_size)
    else:
        train_loader = DataLoader(PointCloudDataset(
            args.csvPath, transforms=train_transforms, mode="train"), batch_size=args.batch_size)

    val_loader = DataLoader(PointCloudDataset(
            args.csvPath, transforms=False, mode="test"), batch_size=args.batch_size)
    ...
    if args.expr_type == "wm":
        n_samples = torch.tensor(
            cfg.class_weights, dtype=torch.float, device=args.gpu)
    ...
    for epoch in range(first_epoch, args.epochs+1):
        ...
        print()
        print('Accuracy     ', *
            [f'{i:>5d}' for i in range(num_classes)], '   OA', sep=' | ')
        print('Training:    ', *[f'{acc:.3f}' if not np.isnan(acc)
            else '  nan' for acc in accs], sep=' | ')
        print('Validation:  ', *[f'{acc:.3f}' if not np.isnan(acc)
            else '  nan' for acc in val_accs], sep=' | ')

        print('IoU          ', *
            [f'{i:>5d}' for i in range(num_classes)], ' mIoU', sep=' | ')
        print('Training:    ', *[f'{iou:.3f}' if not np.isnan(iou)
            else '  nan' for iou in ious], sep=' | ')
        print('Validation:  ', *[f'{iou:.3f}' if not np.isnan(iou)
            else '  nan' for iou in val_ious], sep=' | ')

        print('Time elapsed:', '{:.0f} s'.format(
            d) if d < 60 else '{:.0f} min {:02.0f} s'.format(*divmod(d, 60)))
