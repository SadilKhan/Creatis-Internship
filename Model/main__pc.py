...
from Codes.Datagen.globalVar import LABEL_TO_ORGAN
...
class PointCloudDataset:
    def __init__(self,path,test_size):
        self.name = 'PointCloudDataset'
        self.path = path
        self.label_to_names = LABEL_TO_ORGAN
        self.num_classes = len(self.label_to_names)
        self.label_values = np.sort([k for k, v in self.label_to_names.items()])
        self.label_to_idx = {l: i for i, l in enumerate(self.label_values)}
        self.ignored_labels = np.sort([0])
        self.test_size=1-self.test_size
        ...
        self.val_files = []
        self.test_files = []
        cloud_names = os.listdir(self.path)
        
        self.train_files,self.test_files=train_test_split(cloud_names,test_size=self.test_size)
        ...
        self.class_weight = {}
        self.input_trees = {'training': [], 'test': []}
        self.input_colors = {'training': [], 'test': []}
        self.input_labels = {'training': [], 'test': []}
