...
"""
import sys,os
sys.path.append("/home/khan/Internship/Model/model_utils")

import torch
from model_utils.tools import calculate_weight
...
import numpy as np
from tools import calculate_weight
...
class GDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        # copy from https://github.com/LIVIAETS/surface-loss/blob/108bd9892adca476e6cdf424124bc6268707498e/losses.py#L29
        w: torch.Tensor = 1 / (einsum("bch->bh", y_onehot).type(torch.float32) + 1e-10)**2
        intersection: torch.Tensor = w * einsum("bcw, bcw->bw", net_output, y_onehot)
        union: torch.Tensor = w * (einsum("bch->bh", net_output) + einsum("bch->bh", y_onehot))
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, weights, aggregate="sum"):
    ...
    def forward(self, net_output, target):
        probScores=nn.Softmax(dim=1)(net_output)
        probScores=probScores.permute(0,2,1)
        dc_loss = self.dc(probScores[0], target[0])
