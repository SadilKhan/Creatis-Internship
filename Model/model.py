...
class RandLANet(nn.Module):
    ...
    def forward(self, input,nbr_intensity):
        ...
        #x=input.unsqueeze(-1).permute(0,2,1,3)
        x=torch.cat((x,features),dim=1) # shape (B, d+d1, N, 1)
        """x=self.fc_start(x).transpose(-2,-1).unsqueeze(-1)
        x=self.bn_start(x) # shape (B, d, N, 1)"""
        ...
        """idx,dist=findKNN(coords,coords,4)
        B, N, K = idx.size()
        expanded_idx = idx.unsqueeze(1).expand(B, emb.size(1), N, K)
        expanded_features = emb.expand(B, -1, N, K)
        # Fee Cached Tensor
        del emb,x
        gc.collect()
        torch.cuda.empty_cache()

        neighbor_features = torch.gather(expanded_features.cpu(), 2, expanded_idx) # (B,-1,N,K)

        pooledEMB=torch.mean(neighbor_features,dim=-1) # (B,128,N)
        pooledEMB=pooledEMB.unsqueeze(-1) # (B,128,N,1)

        pooledEMB=pooledEMB.cuda()"""
        scores=self.fc_output(emb)

        return scores.squeeze(-1),emb
