...
def evaluate(model, loader, criterion, device,output):
    ...
    f1s=[]
    with torch.no_grad():
        for name,points,nbr_intensity, labels in tqdm(loader, desc='Validation', leave=False):
            points = points.to(device)
            labels = labels.to(device)
            nbr_intensity=nbr_intensity.to(device)
            scores,_ = model(points,nbr_intensity)
            #probScores = nn.Softmax(dim=1)(scores)[0]
            loss = criterion(scores,labels)
            losses.append(loss.cpu().item())
            accuracies.append(accuracy(scores, labels))
            #f1s.append(accuracy(scores, labels)[1])
...
def train(args,csvFiles, train_transforms=False, load=False):
    ...
    for epoch in range(first_epoch, args.epochs+1):
        ...
        for name,points,nbr_intensity, labels in tqdm(train_loader, desc='Training', leave=False):
        ...
        #f1s=np.nanmean(np.array(f1s), axis=0)
        ...
        """f1_dicts=[
            {
                'Training accuracy': f1s,
                'Validation accuracy': val_f1s
            } for f1, val_f1 in zip(f1s, val_f1s)
        ]"""
        ...
        """print('F1           ', *
            [f'{i:>5d}' for i in range(num_classes)], ' mF1', sep=' | ',file=output)
        print('Training:    ', *[f'{f1:.3f}' if not np.isnan(f1)
            else '  nan' for f1 in f1s], sep=' | ',file=output)
        print('Validation:  ', *[f'{f1:.3f}' if not np.isnan(f1)
            else '  nan' for f1 in val_f1s], sep=' | ',file=output)   """ 
